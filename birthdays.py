...
def get_birthdays_per_week(users, detailed_info=False):
    ...
    for user in users:
        ...
        bd_for_next_week_starts_in_days = 7 - today.weekday() - 2 # this weekend goes to the next week
        bd_for_next_week_ends_in_days   = bd_for_next_week_starts_in_days + 7
        if bd_for_next_week_starts_in_days <= delta_days < bd_for_next_week_ends_in_days:
            congrats_at = birthday_this_year.weekday()
            if congrats_at > 4: # BD at weekend (days 5 and 6) will congratulate at Monday (day 0)
                congrats_at = 0
            if detailed_info:
                print(f"BD at {user['birthday'].date()}, in {delta_days:>2} days ({day_name[birthday_this_year.weekday()]:>9}), congrats at {day_name[congrats_at]:>9}")
...
if __name__ == '__main__':
    ...
    def __generate_user_birthdays_test_data(num: int, add_corner_cases = False):
        ...
        fake = Faker()
        for i in range(num):
            d = {"name": fake.name(), "birthday": datetime.combine(fake.date_between('-80y'), time())}
            data.append(d)
        if add_corner_cases:
            ...
            for i in range(-3, 15):
                for iters in range(randint(1,3)):
                    ...
                    d = {"name": fake.name(), "birthday": datetime.combine(date, time())}
                    data.append(d)
        return data
