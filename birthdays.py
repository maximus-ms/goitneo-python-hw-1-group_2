...
if __name__ == "__main__":
    ...
    def __get_today(args):
        ...
        try:
            if len(args) == 0 or not args[0].isdigit(): today = None
            elif len(args) <= 2: today = fake_today[int(args[0])]
            elif len(args) >= 3: today = date(int(args[0]), int(args[1]), int(args[2]))
        except BaseException as e:
            print("Error to parse args:", type(e).__name__)
            exit()
        return today
    ...
    def __get_users(args, users_number, in_today):
        ...
        if len(args) == 0 or args[-1].isdigit():
            users = __generate_user_birthdays_test_data(users_number, add_corner_cases=True, today=in_today)
        else:
            fname = args[-1]
            try:
                with open(fname, 'r') as file:
                    for in_data in file.readlines():
                        in_data = in_data.replace("'",'"').replace('datetime.','').replace('datetime','').replace("(",'"').replace(")",'"')
                        in_dict = json.loads(in_data)
                        in_date = in_dict["birthday"].split(',')
                        in_date = date(int(in_date[0]), int(in_date[1]), int(in_date[2]))
                        in_dict["birthday"] = datetime.combine(in_date, time())
                        users.append(in_dict)
            except BaseException as e:
                print(f"Error to parse input file '{fname}':", type(e).__name__)
                exit()
        return users
